...
static struct array *allthreads;

struct thread **process_table;
struct lock *process_pid;
struct lock *execv_lock; //remaining
...
thread_create(const char *name)
{
	...
	thread->number_of_threads_waiting_on_you=0;
	...
	thread->parent_pid = -1;
	thread->priority = 0;
	thread->time_slice = (thread->priority) + 1;
	thread->children=NULL;
	thread->curr_state = ALIVE;  
	return thread;
}
...
thread_destroy(struct thread *thread)
{
	...
	/*  Remove thread from the list of existing threads */
	for(i = 0; i < array_getnum(allthreads); i++) {
		if(thread->pid == ((struct thread*) array_getguy(allthreads, i))->pid) {
			array_remove(allthreads,i);
		}
	}
...
thread_bootstrap(void)
{
	struct thread *me;
	int max = 32769;
	process_table = kmalloc(max*sizeof(struct thread *)); 

	
	process_pid = lock_create("process_pid");
	if (process_pid == NULL)
		panic("thread_bootstrap: Out of memory\n");

	execv_lock = lock_create("execv_lock");
	if (execv_lock == NULL)
		panic("thread_bootstrap: Out of memory creating execv_lock\n");
...
/*  Check if a pid has already been in use*/
int test_pid_allocated(pid_t pid) {
	int i;
	for(i = 0; i < array_getnum(allthreads); i++) {
		if(pid == ((struct thread*) array_getguy(allthreads, i))->pid) {
			return 1;
		}
	}
	return 0;
...
/*  Allocate pid to a process */
pid_t initialize_pid(void) {
	pid_t process;
	lock_acquire(process_pid);
	do{
		process = (pid_t) random()%32768;
	} while(test_pid_allocated(process));
	lock_release(process_pid);
	return process;
}
	    ...
	    struct thread **ret)
{
	...
	/*  Allocate  pid to this thread */
	newguy->pid = initialize_pid();
	/*
 ...
 *  	  	  	 */
	...
	result = array_preallocate(allthreads, numthreads+1);
	if (result) {
		goto fail;
	}
...
if (curthread->t_vmspace != NULL) {
		...
		err = as_copy(curthread->t_vmspace,&(newguy->t_vmspace));                
                #else
                err = as_copy(curthread->t_vmspace,&(newguy->t_vmspace), newguy->pid);                
                #endif  
	}    
	/*
 ...
 *   	   	   	   	   	 */
	...
	DEBUG(DB_THREADS, "Forking new thread: %s. PID is %d\n", name, newguy->pid);

	/*  Add the new thread to the list of existing threads */
	array_add(allthreads, newguy);
...
int
thread_fork_userthread(const char *name, 
	    void *data1, unsigned long data2,
	    void (*func)(void *, unsigned long),
	    struct thread **ret)
{
	...
	newguy->is_user_thread=1;
	newguy->parent_pid = curthread->pid;
	newguy->waiting_on = sem_create("waiting_on",0);
 ...
 *  	  	  	 */
	...
	if (nextstate==S_READY) {
		...
		/* The thread ran out of the time slice, & is yet not finished.
 *   		 * we increase the priority levels, to decrease the priority.
 *   		  		 */

		if(cur->priority < 20 -1) {
			int temp = cur->priority;
			cur->priority = temp +1;
			cur->time_slice = (cur->priority + 1) * 2;
		}
 ...
 *  		  		  		 */
		...
		/* Again, allocate the time for running state */
 ...
 *	      	  	 */
	...
	int do_while= curthread->number_of_threads_waiting_on_you;
	while(do_while!=0)
	{
		V(curthread->waiting_on);
		do_while= do_while-1;
	}
	if(curthread->is_user_thread ==1)
	{
        V(curthread->waiting_on);
	}
